...
export default function ProductManagementPage() {
  ...
  const getFinalPrice = (p: Product) => {
    ...
    if (p.discountActive && p.discountPercentage && p.discountPercentage > 0) {
      return Number(p.price) - (Number(p.price) * Number(p.discountPercentage)) / 100;
    }
  ...
  const toggleStock = async (p: Product) => {
    ...
    try {
      ...
      setProducts(prev =>
        prev.map(pr => pr.id === p.id ? { ...pr, inStock: !p.inStock } : pr)
      );
    } catch {
                  ...
                  <div className="form-group pm-checkbox-group">
                    <label>
                      <input
                        ...
                        onChange={e => setField('discountActive', e.target.checked)}
                      />
                      Discount Active
                    </label>
                  </div>
                ...
                <table className="admin-table">
                  ...
                  <tbody>
                    {filtered.map(p => {
                      ...
                      const hasDiscount = p.discountActive && Number(p.discountPercentage || 0) > 0;
                      ...
                      return (
                        <tr key={p.id}>
                          ...
                          <td>
                            {hasDiscount ? (
                              <span className="pm-badge-label">{p.discountPercentage}% OFF</span>
                            ) : (
